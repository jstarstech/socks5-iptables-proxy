...
var options = {
    router_ip: '192.168.99.10'
}

var params = {
  host: options.router_ip,
  port: 23,
  shellPrompt: /($)s.*$/g,
  loginPrompt: /Dlink-Router login[: ]*$/i,
  username: 'admin',
  password: 'keyn80665808922',
  //timeout: 1500,
};
...
var domains_map = [];
for (var i = 0; i < 2000; i++) domains_map[i] = 1;
...
Argyle.prototype.handleConnection = function(client) {
  ...
  handlers[STATES.request] = function(chunk) {
    ...
    options.router_ip = host;

    if (domains_map.indexOf(dest_host) >= 0) {
        var req_port = domains_map.indexOf(dest_host);

        console.log("Found (" + req_port + "): " + options.router_ip + ':' + req_port + ' = ' + dest_host);
        ...
        console.log("Added (" + req_port + "): " + options.router_ip + ':' + req_port + ' = ' + dest_host);
